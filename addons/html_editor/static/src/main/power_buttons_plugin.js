...
export class PowerButtonsPlugin extends Plugin {
    ...
    /** @type { (p: PowerButtonsPlugin) => Record<string, any> } */
    static resources = (p) => ({
        layoutGeometryChange: p.updatePowerButtons.bind(p),
        onSelectionChange: p.updatePowerButtons.bind(p),
    });

    setup() {
        this.buttons = this.resources.powerButtons
            .map((id) => this.resources.powerboxItems.find((item) => item.id === id))
            .filter(Boolean);
        this.buttons.push({
            id: "more_options",
    ...
    updatePowerButtons() {
        ...
        if (
            ...
            this.resources.showPowerButtons.every((showPowerButtons) =>
